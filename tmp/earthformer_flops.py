...
if test_dataset_name == "earthnet2021":
    ...
    cfg_file_path = os.path.join(os.path.dirname(__file__), "earthformer_flops_cfg.yaml")
else:
...
if test_dataset_name == "earthnet2021":
    def get_model_config():
        ...
        cfg.initial_downsample_stack_conv_downscale_list = [2, 2, 2]
...
flops = FlopCountAnalysis(model=model.to(device),
                          inputs=in_seq)
output = model.to(device)(in_seq, verbose=True)

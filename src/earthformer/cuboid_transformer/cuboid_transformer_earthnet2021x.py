...
from ..datasets.earthnet.conditional_layer import (
...
class CuboidTransformerEncoderEarthNet2021x(nn.Module):
    """Encoder of the CuboidTransformer adapted for EarthNet2021x
    ...
    def __init__(self,
        ...
        padding_type
        """
        super(CuboidTransformerEncoderEarthNet2021x, self).__init__()
        # initialization mode
...
class CuboidTransformerDecoderEarthNet2021x(nn.Module):
    """Decoder of the CuboidTransformer adapted for EarthNet2021x.
    ...
    def __init__(self,
        ...
        checkpoint_level
        """
        super(CuboidTransformerDecoderEarthNet2021x, self).__init__()
        # initialization mode
        self.attn_linear_init_mode = attn_linear_init_mode
        self.ffn_linear_init_mode = ffn_linear_init_mode
        self.conv_init_mode = conv_init_mode
...
class CuboidTransformerModelEarthNet2021x(nn.Module):
    """Cuboid Transformer adapted for EarthNet2021x for spatiotemporal forecasting
    ...
    def __init__(self,
                 ...
                 weather_conditioning_channels=1,
                 ):
        """
        ...
        """
        super(CuboidTransformerModelEarthNet2021x, self).__init__()
        # initialization mode
        self.attn_linear_init_mode = attn_linear_init_mode
        self.ffn_linear_init_mode = ffn_linear_init_mode
        self.conv_init_mode = conv_init_mode
        ...
        elif weather_conditioning_loc == "early":
            if weather_conditioning == "cat":
                self.early_conditioning = Cat_Project_Conditioning(x_channels=x_channels,
                                                                   c_channels=c_channels)
            elif weather_conditioning == "FiLM":
                self.early_conditioning = FiLMBlock(n_c=c_channels,
                                                    n_hid=hidden_channels,
                                                    n_x=x_channels)
            elif weather_conditioning == "xAttn":
                self.early_conditioning = CrossAttention_Conditioning(x_channels=x_channels,
                                                                      c_channels=c_channels,
                                                                      n_tokens_c=1, act="leakyrelu",
                                                                      hidden_dim=hidden_channels, n_heads=num_heads,
        ...
        elif weather_conditioning_loc == "latent":
            if weather_conditioning == "cat":
                self.early_conditioning_before = Cat_Project_Conditioning(x_channels=x_channels,
                                                                          c_channels=c_channels)
            elif weather_conditioning == "FiLM":
                self.early_conditioning_before = FiLMBlock(n_c=c_channels,
                                                           n_hid=hidden_channels,
                                                           n_x=x_channels)
            elif weather_conditioning == "xAttn":
                self.early_conditioning_before = CrossAttention_Conditioning(x_channels=x_channels,
                                                                             c_channels=c_channels,
                                                                             n_tokens_c=1, act="leakyrelu",
                                                                             hidden_dim=hidden_channels, n_heads=num_heads,
                                                                             mlp_after_attn=False)
        ...
        self.decoder = CuboidTransformerDecoderEarthNet2021x(
        ...
        if weather_conditioning_loc in ["all", "early"]:
            ...
            if weather_conditioning == "cat":
                self.early_conditioning_after = Cat_Project_Conditioning(x_channels=x_channels,
                                                                         c_channels=c_channels)
            elif weather_conditioning == "FiLM":
                self.early_conditioning_after = FiLMBlock(n_c=c_channels,
                                                          n_hid=hidden_channels,
                                                          n_x=x_channels)
            elif weather_conditioning == "xAttn":
                self.early_conditioning_after = CrossAttention_Conditioning(x_channels=x_channels,
                                                                            c_channels=c_channels,
                                                                            n_tokens_c=1, act="leakyrelu",
    ...
    def forward(self, x, cond=None, verbose=False):
        """

...
pytorch_state_dict_name = "earthformer_earthnet2021x_cond.pt"
...
class CuboidEarthNet2021xPLModule(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 oc_file: str = None,
                 save_dir: str = None):
        super(CuboidEarthNet2021xPLModule, self).__init__()
    ...
    def get_logging_config():
        cfg = OmegaConf.create()
        cfg.logging_prefix = "EarthNet2021x"
        cfg.monitor_lr = True
    ...
    @staticmethod
    def get_earthnet2021x_datamodule(
            dataset_cfg,
    ...
    def forward(self, batch):
        ...
        # mask from updated EarthNet2021x: non-0 for mask and 0 for non-masked.
        mask = rearrange(batch["dynamic_mask"][0],
                         f"{' '.join(self.oc.dataset.layout)} -> {' '.join(self.layout)}")
        if not self.oc.dataset.use_mask:
            mask = torch.ones_like(mask)
        else:
            mask = 1.0 - mask.bool().float()  # original val is 0, 1, 2, 3, 4.
        ...
        mesodynamic = rearrange(batch["dynamic"][1], "b t c -> b t 1 1 c")
        aux_data = mesodynamic

        pred_seq = self.torch_nn_module(model_in, aux_data[self.in_slice], aux_data[self.out_slice])
...
def get_parser():
    ...
    parser.add_argument('--save', default='tmp_earthnet2021x', type=str)
...
def main():
    ...
    if args.cfg is not None:
        ...
        dataset_cfg = OmegaConf.to_object(CuboidEarthNet2021xPLModule.get_dataset_config())
        micro_batch_size = 1
        total_batch_size = int(micro_batch_size * args.gpus)
        max_epochs = None
        seed = 0
    seed_everything(seed, workers=True)
    dataloader_dict = CuboidEarthNet2021xPLModule.get_earthnet2021x_dataloaders(
        dataset_cfg=dataset_cfg,
        micro_batch_size=micro_batch_size,
        num_workers=8, )
    accumulate_grad_batches = total_batch_size // (micro_batch_size * args.gpus)
    total_num_steps = CuboidEarthNet2021xPLModule.get_total_num_steps(
        epoch=max_epochs,
        num_samples=dataloader_dict["num_train_samples"],
        total_batch_size=total_batch_size,
    )
    pl_module = CuboidEarthNet2021xPLModule(
